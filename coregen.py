...
for (f,ip) in vfunc_ipname:
	...
	if ip_master_conn:
		...
		connections.update({ ip_master_conn[0][0] : [ip_master_conn[0][1]]})
	else:
		to_int = 'BUS'
...
bus_ip_connections = filter(lambda x: (x[0][0]=='BUS' or x[0][0] not in ip_hw_name) and x[1][0] in ip_hw_name, ip_connections)
if bus_ip_connections:
	l = []
	for b in bus_ip_connections:
		l.append(b[1])
	connections.update({ ('BUS',axiLite_interface) : l })

ip_bus_connections = filter(lambda x: (x[1][0]=='BUS' or x[1][0] not in ip_hw_name) and x[0][0] in ip_hw_name, ip_connections)
if ip_bus_connections:
	for b in ip_bus_connections:
		connections.update({ b[0] : [('BUS',axiLite_interface)] })


print "CONNECTIONS : \n", connections,'\n'

gen_tcl_sys(coreTEST, connections, vfunc_ipname)
